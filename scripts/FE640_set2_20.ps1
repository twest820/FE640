...
$tabuAndObjectives = Optimize-Tabu -BestOf 1000 -UniformHarvestProbability -Units $units -Iterations 275 -MaximumUnitIndex 2000 -Tenure 5 -TargetHarvestPerPeriod 439620 -TargetHarvestWeights $harvestWeights -Verbose
...
$delugeAndObjectives = Optimize-GreatDeluge -BestOf 1000 -InitialWaterLevelMultiplier 1.02 -RainRate 0.99997 -StopAfter 1E6 -UniformHarvestProbability -Units $units -TargetHarvestPerPeriod 439620 -TargetHarvestWeights $harvestWeights -Verbose;
...
$recordTravelAndObjectives = Optimize-RecordTravel -BestOf 1000 -Deviation 0.9E6 -StopAfter 5E5 -UniformHarvestProbability -Units $units -TargetHarvestPerPeriod 439620 -TargetHarvestWeights $harvestWeights -Verbose;
$recordTravelAndObjectives[0].BestHarvestByPeriod
Write-Harvest -Heuristics $recordTravelAndObjectives[0] -CsvFile ([System.IO.Path]::Combine($buildDirectory, "FE640_set2_20_rt1k2_harvest.csv"));
Write-HarvestSchedule -Heuristics $recordTravelAndObjectives[0] -CsvFile ([System.IO.Path]::Combine($buildDirectory, "FE640_set2_20_rt1k2_schedule.csv"));
Write-Objective -Heuristics $recordTravelAndObjectives[0] -CsvFile ([System.IO.Path]::Combine($buildDirectory, "FE640_set2_20_rt1k2_objective.csv"));
$recordTravelAndObjectives[1] | Out-File -FilePath ([System.IO.Path]::Combine($buildDirectory, "FE640_set2_20_rt1k2_objectiveDistribution.csv")) -Encoding utf8;