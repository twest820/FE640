...
$annealerAndObjectives = Optimize-SimulatedAnnealing -BestOf 100 -Units $units -TargetHarvestPerPeriod 440500 -Verbose;
...
Write-HarvestSchedule -Heuristics $annealerAndObjectivesgG[0] -CsvFile ([System.IO.Path]::Combine($buildDirectory, "FE640_set1_20_sa_schedule.csv"));
...
$acceptor = Optimize-ThresholdAccepting -Units $units -TargetHarvestPerPeriod 440500 -Thresholds (1.05, 1.03, 1.00) -Verbose;
$acceptor.BestHarvestByPeriod
$acceptor.CurrentHarvestByPeriod
$acceptorAndObjectives = Optimize-ThresholdAccepting -BestOf 100 -Units $units -TargetHarvestPerPeriod 439600 -Thresholds (1.05, 1.03, 1.00) -Verbose;
